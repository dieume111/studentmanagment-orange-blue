...
<div style="display: flex; justify-content: center; align-items: center; height: 100vh;">
    <div style="background: white; padding: 20px; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1); width: 300px;">
        <h2 style="text-align: center; color: #ff6200;">Register</h2>
        <form method="POST" action="{{ route('register') }}">
            ...
            @error('password')
                <p style="color: red; font-size: 12px;">{{ $message }}</p>
            @enderror
            <button type="submit" style="width: 100%; padding: 10px; background: #007bff; color: white; border: none; border-radius: 4px; cursor: pointer;">Register</button>
        </form>
        <p style="text-align: center; margin-top: 10px;">Already have an account? <a href="{{ route('login') }}" style="color: #ff6200;">Login</a></p>
    </div>
</div>
@endsection